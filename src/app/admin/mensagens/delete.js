...
const DeleteForm = ({ data, setViewModalDelete }) => {
    ...
    const handleDeleteForm = async () => {
        ...
        try {
            const response = await Api.post("/admin/mensagens/deletar", {
                token: loginToken,
                id: routeData.id,
            });
            ...
            if (response.data) {
                ...
                if (response.data.status) {
                    refreshRouter("/admin/mensagens", 1500);
                }
            }
        } catch (error) {
            setLoading(false);
        ...
        <div>
            ...
            {routeData && (
                <div className="mt-3 mb-5 p-3 border bg-zinc-100 rounded-lg">
                    ...
                    <p className="text-sm mb-1 font-semibold">Nome: {routeData.name}</p>
                    <p className="text-sm mb-1 font-semibold">Subject: {routeData.subject}</p>
                </div>
            )}
