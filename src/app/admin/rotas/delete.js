...
const DeleteForm = ({ data, setViewModalDeleteRoutes }) => {
    ...
    const handleDeleteForm = async () => {
        ...
        try {
            const response = await Api.post("/admin/rotas/deletar", {
                token: loginToken,
                id: routeData.id,
            });
        ...
        <div>
            ...
            {routeData && (
                <div className="mt-3 mb-5 p-3 border bg-zinc-100 rounded-lg">
                    ...
                    <p className="text-sm font-semibold">URI: {routeData.uri}</p>
                </div>
            )}

            <div className="flex justify-end items-center gap-3">
                <ButtonLink color="text-white bg-rose-600" title="Excluir Rota" onClick={handleDeleteForm} />
            </div>
        </div>
    );
};
