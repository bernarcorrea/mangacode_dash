...
const Form = ({ data, setViewModal }) => {
    ...
    const handleSubmitForm = async () => {
        ...
        try {
            const response = await Api.post("/admin/controladores/manager", {
                token: loginToken,
                id: fieldId,
                title: fieldTitle,
            });
            ...
            if (response.data) {
                ...
                if (response.data.status) {
                    refreshRouter("/admin/controladores", 1500);
                }
            }
        } catch (error) {
            setLoading(false);
            ...
            <form className="mt-5">
                <div className="mb-4">
                    <InputText
                        title="Título"
                        type="text"
                        placeholder="Título do controlador"
                        required={true}
                        value={fieldTitle}
